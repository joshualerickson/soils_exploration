...
wild_rice_steep %>% filter(mean_deficit > 300) %>%  st_intersection(landtype) %>% ggplot(aes(mean_deficit, mean_npp) ) + geom_point(aes(color = factor(LANDTY)), position = position_dodge(10), size = 4) + geom_smooth(aes(mean_deficit, mean_npp), method = "lm", alpha = 0.1) +
  ...
  labs(title = "Wild Rice Units > 300 CWD", x = "Mean Deficit of Unit", y = "Mean NPP", color = "Landtype") +
...
```{r eval=FALSE, echo=FALSE}
pca_ltypes <- NHDcatchmentLtypes %>% st_drop_geometry()
pca_ltypes <- pca_ltypes[,c(9:13,15:19,36)]
pca_ltypes <- na.omit(pca_ltypes)
pca_ltypes <- pca_ltypes %>% filter(as.integer(SoilClass) != 6)
cor_table <- data.frame(round(cor(pca_ltypes), 2))


```
```{r eval=FALSE, echo=FALSE, fig.width=1, fig.height=5}
names(cor_table) <- pander::pandoc.strong.return(names(cor_table))
pander::pander(cor_table, caption = "Cabinet District Correlation Table", emphasize.strong.cols = 1, split.cells = c(rep(1,10)))
```

```{r eval=FALSE, echo=FALSE}
pca_sum <- prcomp(pca_ltypes[,-11], center = TRUE, scale. = TRUE)
pca_sum_veg <- prcomp(pca_ltypes[,c(1,4,5,6,7)], center = TRUE, scale. = TRUE)
summary(pca_sum)
pca_sum$rotation
```
```{r eval=TRUE, echo=FALSE}
pander::pander(summary(pca_sum))
wild_rice_steep %>% dplyr::group_by(Landtype) %>% dplyr::count(Landtype, wt = Acres)
```

```{r eval=TRUE, echo=FALSE}
ggbiplot::ggbiplot(pca_sum, ellipse = TRUE, groups = pca_ltypes$SoilClass)
